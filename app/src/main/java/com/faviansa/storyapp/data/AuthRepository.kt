...
class AuthRepository private constructor(private val apiService: ApiService, private val preferences: StoryAppPreferences) {
        ...
        try {
            ...
            when {
                response.isSuccessful -> {
                    ...
                    emit(Result.Error(response.errorBody()?.string() ?: "Unknown error occurred"))
                }
            }
        } catch (e: Exception) {
    ...
    fun login(email: String, password: String): Flow<Result<LoginResponse>> = flow {
        ...
        try {
            ...
            when {
                response.isSuccessful -> {
                    response.body()?.let { data ->
                        preferences.saveToken(data.loginResult?.token ?: "")
                        emit(Result.Success(data))
                    }
                }
                else -> {
